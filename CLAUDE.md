...
src/claude_pilot/transcript_writer.py → cpp#165 side-channel JSONL transcript into $ANTHROPIC_LOG_FILE (read by mika)
...
- **System prompt is preset+append, never a plain string (mika#1409).** `agent.py` sets `ClaudeAgentOptions.system_prompt` to a `SystemPromptPreset` — `{"type": "preset", "preset": "claude_code", "append": DENIED_BASH_PATTERNS_HINT}`. The preset+append shape is load-bearing: a plain-string `system_prompt` would REPLACE the Claude Code preset and break the headless `/mika` + `/ce:*` pipeline that depends on it. The appended hint (defined in `tier1.py` next to the deny-list patterns it mirrors, so it cannot drift) names the most-commonly-denied Bash patterns (`find … -exec`, non-safe-listed tools like `md5sum`, `sed -i`, `>` redirects) and their auto-approved native-tool substitutes, so the model avoids reaching for a policy-denied command. Post-cpp#128 a denial no longer halts the session by default — `interrupt=True` is reserved for a destination veto (worktree containment) and tier3-dangerous Bash (`sed -i`, `eval`, `bash -c`, `sh -c`, and a `>` redirect whose target is NOT a contained working file — see cpp#154 below), so of the patterns the hint names, `find … -exec` and `md5sum` are refused without ending the run. The recoverable-denial half asked for in mika#1410 shipped with cpp#128; the hint remains worthwhile because a reach that never happens costs no turn at all.
- **A survivable refusal no longer ends the run either (cpp#151).** cpp#128 split the *decision* from the *lethality* and drove a refusal's measured lethality from 100% to 32% — but not to 0. The residue: a refusal `_denial_is_terminal` classified NON-terminal, handed to the model as a `tool_result`, followed by an `error_during_execution` from the SDK's own bundled `claude` binary (`stop_reason=tool_use`, `last_content_type=n/a`). Three additive pieces close it, and none of them relaxes a permission. (1) **Lethality is now logged**: `ui.log_policy_deny` takes a required keyword-only `terminal` and renders `(terminal)` / `(non-terminal)`, so the population "sessions that took a survivable refusal" is readable from `/var/log/claude-pilot/*.stderr` instead of being a retrospective guess; the same verdict rides the cm#99 audit wire as a seventh field. (2) **`guardrails.note_policy_deny`** arms one of two sticky session markers — `nonterminal_policy_deny` for a survivable refusal, `terminal_policy_deny` for a deliberately lethal one. (3) **`agent._DenyResumeController`** spends a bounded, always-logged budget (`CLAUDE_PILOT_MAX_DENY_RESUMES`, default 2, ceiling 5, `0` disables) on the SDK's **session-resume** path — a NEW `ClaudeSDKClient` carrying `ClaudeAgentOptions.resume=<session id>`, opened by the session loop in `_run_agent_inner`, given a follow-up prompt that restates the refusal stands. It is emphatically *not* a second `query()` on the same client: when the CLI emits an error result *"it then exits non-zero on purpose"* (`claude_agent_sdk/_internal/query.py`), and the reader queues the trailing `ResultError` for whoever reads next — so `_merge_stream` must never read past a `ResultMessage`, and does not. When the resume declines, is spent, or fails to come up, the death is named `error_during_execution:after_deny` rather than sharing a line with every other SDK-loop failure. **Two independent vetoes protect the lethal classes:** a sticky `guardrails.terminal_policy_deny` (any deliberately lethal refusal disqualifies the run permanently) and the SDK's own `ResultMessage.terminal_reason in {aborted_tools, aborted_streaming}`. Both are needed because the survivable marker is *sticky*: without the veto, one harmless early refusal would have made every later containment kill resumable. Tests pin both directions.
- **A denial that only writes a working file is survivable (cpp#154).** cpp#130 removed ONE redirect target from the lethal class — the inert `/dev/null` sink. cpp#154 removed the rest of the contained ones. A refusal whose CAUSE is pure FORM (a chain `_bash_allow_is_chain_safe` cannot honour) no longer ends the run merely because the command writes a file: `is_tier3_dangerous_for_lethality` now blanks any redirect whose LITERAL target is under `/tmp/` or relative to the worktree (no `..` anywhere, no leading `~` or `$`) before the tier3 pattern test. The refusal is untouched — `is_tier3_dangerous` has no diff — and `_denial_is_terminal` remains the single consumer, per cpp#151 B0. Evidence: three pilots on mika#2158 died in one day (2026-09-04) on this class, the last after 155 turns and 8 pushed commits with the PR one call away. **Two guards keep AC2 intact and both are load-bearing.** The lexical test is cwd-free by design (it must be: cpp#143 broke the cpp#38 symlink tests by resolving in order to GRANT an exemption), so it structurally cannot tell `> notes.txt` from `> .git/hooks/pre-commit` or from `> esc/x` through a symlink — until cpp#154 the blanket `>` pattern covered both by accident. `permissions._redirect_destination_veto_reason` restores them on purpose, on the lethality path ONLY, by resolving to WITHHOLD lethality rather than to grant an exemption. It is deliberately not a `_segment_write_kind` entry: that function is also consulted on the ALLOWED path, where a redirect write-kind would veto `cat > /tmp/x <<'EOF'` and regress `bash-cat-heredoc-tmp` (deferred as claude-pilot#155).
- **Pilot-transcript side-channel (cpp#165 / mika#2040).** One JSONL line per COMPLETE SDK message (`AssistantMessage`, `ResultMessage`), appended to `$ANTHROPIC_LOG_FILE` by `transcript_writer.record_sdk_message`. The hook is a single call at the **top** of `agent.py`'s `_merge_stream` loop, before every type branch: the `ResultMessage` branch can `break` early on cpp#151's deny-resume, so a writer inside it would lose precisely the sessions that died on a refusal. `StreamEvent` is excluded on purpose — it carries the same tokens as the turn-closing `AssistantMessage`, and recording both would double-count. **This is one third of a cross-repo contract**: mika `skills/executor.rs` injects the env var and mounts the directory (mika#1705), and mika `crates/mika-agent/src/task_engine/pilot_transcript.rs` reads the lines back and is the sole source of truth for the format (mika#2040 / PR#2240). `schema_version: "v1"` is mandatory on every line and an unknown version is refused wholesale on the mika side, so bumping it means shipping the reader's dual-accept release first. `request_body` stays null: the outbound HTTP body is not exposed at the SDK consumption point despite the env var's name. Fails open like the inbox writer — an unwritable path logs once, disarms for the session, and never changes the exit code.
...
### Pilot transcript (cpp#165)

- `ANTHROPIC_LOG_FILE` — absolute path to the session's JSONL transcript, injected per dispatch by mika as `~/.mika/data/pilot-transcripts/<task-id>.jsonl`. Unset or blank → the writer is a silent no-op, which is the normal state outside a mika dispatch.
